...
const config = {
  headers: {
    Authorization: `Bearer eiahvbNv4kPahHyfosgAfb1JnQRp1IUy`,
  },
};
...
const fetchDex = async () => {
  const allowance = await axios.get(`https://api.1inch.dev/swap/v5.2/1/approve/allowance?tokenAddress=${tokenOne.address}&walletAddress=${address}`, config)

  if(allowance.data.allowance === 0) {
    const approve = await axios.get(`https://api.1inch.dev/v5.2/1/approve/transaction?tokenAddress=${tokenOne.address}&amount=100000000000
    `, config)
...
const swap = await axios.get(`https://api.1inch.dev/v5.2/1/swap?fromTokenAddress=${tokenOne.address}&toTokenAddress=${tokenTwo.address}&amount=${tokenOneAmount.padEnd(tokenOne.decimals+tokenOneAmount.length, '0')}&fromAddress=${address}&slippage=${slippage}`, config)
